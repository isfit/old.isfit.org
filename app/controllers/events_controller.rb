   ...
   now = Time.now.strftime("%Y-%m-%d %H:%M:%S")
    ...
    if params[:category]
      ...
      if params[:year] && params[:month] && params[:day]
        ...
        @event_dates = @category.event_dates.joins(:event).all(:conditions=>"event_dates.date LIKE '"+@date.to_s+"%' AND events.visible_at <= '"+now+"'").order("event_dates.date")
      else
      ...
      @date = Date.new(params[:year].to_i, params[:month].to_i, params[:day].to_i)
      @event_dates = EventDate.joins(:event).all(:conditions=>"event_dates.date LIKE '"+@date.to_s+"%' AND events.visible_at <= '"+now+"'").order("event_dates.date")
    else
