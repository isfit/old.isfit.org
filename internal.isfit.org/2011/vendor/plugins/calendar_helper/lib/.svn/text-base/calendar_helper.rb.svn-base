...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    if options[:previous_month_text] or options[:next_month_text]
      cal << %(<th colspan="2">)
      cal << link_to(options[:previous_month_text], :month => options[:month] -1)
      cal << %(</th>)
    ...
    cal << link_to(Date::MONTHNAMES[options[:month]], :controller => "calendar_event", :action => "index",:month => options[:month])
    cal << %(</th>)
    if options[:next_month_text]
      cal << %(<th colspan="2">)
      cal << link_to(options[:next_month_text], :month => options[:month] +1)
      cal << %(</th>)
    end
    ...
    beginning_of_week(first, first_weekday).upto(first - 1) do |d|
      ...
      if options[:accessible]
        cal << %("><span class='hidden'>#{d.day}</span></td>)
      else
        cal << %(">#{d.day}</td>)
      end
...
# ORIG      cal << "<td #{cell_attrs}>#{cell_text}</td>"
      
      cal << "<td #{cell_attrs}>"
      cal << link_to(cell_text, {:controller => "calendar_event", :action => "show", :id => (options[:year].to_s() +"-" + options[:month].to_s() +"-"+cell_text)})
      cal << "</td>"
